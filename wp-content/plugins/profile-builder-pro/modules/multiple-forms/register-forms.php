...
function wppb_create_registration_forms_cpt(){
    $labels = array(
        'name' 					=> __( 'Registration Form', 'profilebuilder'),
        'singular_name' 		=> __( 'Registration Form', 'profilebuilder'),
        'add_new' 				=> __( 'Add New', 'profilebuilder' ),
        'add_new_item' 			=> __( 'Add new Registration Form', 'profilebuilder' ),
        'edit_item' 			=> __( 'Edit the Registration Forms', 'profilebuilder' ) ,
        'new_item' 				=> __( 'New Registration Form', 'profilebuilder' ),
        'all_items' 			=> __( 'Registration Forms', 'profilebuilder' ),
        'view_item' 			=> __( 'View the Registration Form', 'profilebuilder' ),
        'search_items' 			=> __( 'Search the Registration Forms', 'profilebuilder' ),
        'not_found' 			=> __( 'No Registration Form found', 'profilebuilder' ),
        'not_found_in_trash' 	=> __( 'No Registration Forms found in trash', 'profilebuilder' ),
        'parent_item_colon' 	=> '',
        'menu_name' 			=> __( 'Registration Forms', 'profilebuilder' )
    );
...
add_filter( 'wck_update_container_class_wppb_rf_page_settings', 'wppb_register_update_form_change_class_based_on_redirect_field', 10, 4 );
function wppb_register_update_form_change_class_based_on_redirect_field($wck_update_container_css_class, $meta, $results, $element_id) {
...
add_filter( 'wck_element_class_wppb_rf_page_settings', 'wppb_register_form_listing_change_class_based_on_redirect_field', 10, 4 );
function wppb_register_form_listing_change_class_based_on_redirect_field($wck_element_class, $meta, $results, $element_id){
    $redirect = Wordpress_Creation_Kit_PB::wck_generate_slug( $results[$element_id]["redirect"] );
...
function wppb_add_extra_column_for_rf( $columns ){
	$columns['rf-shortcode'] = __( 'Shortcode', 'profilebuilder' );
...
function wppb_rf_side_box(){
	add_meta_box( 'wppb-rf-side', __( 'Form Shortcode', 'profilebuilder' ), 'wppb_rf_content', 'wppb-rf-cpt', 'side', 'low' );	
}
...
function wppb_manage_rf_cpt(){
    ...
    array_unshift( $available_roles , '%'.__( 'Default Role', 'profilebuilder' ).'%'.'default role' );
	...
	$settings_fields = array( 		
		array( 'type' => 'select', 'slug' => 'set-role', 'title' => __( 'Set Role', 'profilebuilder' ), 'options' => $available_roles, 'description' => __( 'Choose what role the user will have after (s)he registered<br/>If not specified, defaults to the role set in the WordPress settings', 'profilebuilder' ) ),
		array( 'type' => 'select', 'slug' => 'automatically-log-in', 'title' => __( 'Automatically Log In', 'profilebuilder' ), 'options' => array( '%'.__('No', 'profilebuilder').'%No', '%'.__('Yes', 'profilebuilder').'%Yes' ), 'default' => 'No', 'description' => __( 'Whether to automatically log in the newly registered user or not<br/>Only works on single-sites without "Admin Approval" and "Email Confirmation" features activated<br/>WARNING: Caching the registration form will make automatic login not work', 'profilebuilder' ) ),
		array( 'type' => 'select', 'slug' => 'redirect', 'title' => __( 'Redirect', 'profilebuilder' ), 'options' => array( '%'.__('Choose...', 'profilebuilder').'%-', '%'.__('No', 'profilebuilder').'%No', '%'.__('Yes', 'profilebuilder').'%Yes' ), 'default' => '-', 'description' => __( 'Whether to redirect the user to a specific page or not', 'profilebuilder' ) ),
		...
		array( 'type' => 'text', 'slug' => 'url', 'title' => __( 'URL', 'profilebuilder' ), 'description' => __( 'Specify the URL of the page users will be redirected once registered using this form<br/>Use the following format: http://www.mysite.com', 'profilebuilder' ) ),
	);
	
	// set up the box arguments
	$args = array(
		'metabox_id' => 'wppb-rf-settings-args',
		'metabox_title' => __( 'After Registration...', 'profilebuilder' ),
	...
	$args = array(
		...
		'meta_array' => $rf_fields = apply_filters	( 'wppb_rf_fields', array( 
																				array( 'type' => 'select', 'slug' => 'field', 'title' => __( 'Field', 'profilebuilder' ), 'options' => $rf_fields, 'default-option' => true, 'description' => __( 'Choose one of the supported fields you manage <a href="'.admin_url( 'admin.php?page=manage-fields' ).'">here</a>', 'profilebuilder' ) ),
...
add_action( 'init', 'wppb_manage_rf_cpt', 10 );
