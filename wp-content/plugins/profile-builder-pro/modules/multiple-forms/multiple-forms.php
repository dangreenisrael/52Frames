...
function wppb_multiple_forms_change_fields( $fields, $args ){
	//if we have a edit_progile form set up the post type and meta name accordingly
	...
	// get all the forms of the perticular form type 
	$get_forms_args = array( 'post_type' => $post_type, 'numberposts' => -1	);						  
	$all_forms = get_posts( $get_forms_args );
	...
	if( !empty( $all_forms ) ){
		foreach( $all_forms as $form ){
			...
			// search for the form that has aur form_name argument
			if( $args['form_name'] == Wordpress_Creation_Kit_PB::wck_generate_slug( $form->post_title ) ){
			
				// let's get the fields that we should display on that form
				$this_forms_fields = get_post_meta( $form->ID, $meta_name, true );
...
function wppb_add_prepopulated_default_fields( $post_id, $post ){
	if ( 'auto-draft' == $post->post_status ) {
		...
		elseif ( 'wppb-epf-cpt' == $post->post_type ){
			// remove reCAPTCHA from the list
			foreach ( $all_fields_new as $key => $value ){
				if ( strpos ( $value['field'], '( reCAPTCHA )' ) !== false ){
...
function wppb_handle_rf_epf_id_change(){
	...
	if ( ( $all_fields != 'not_set' ) && ( ( is_array( $all_fields ) ) && ( !empty( $all_fields ) ) ) ){		
		foreach ( $all_fields as $key => $value ){
			...
			if( wppb_field_format( $value['field-title'], $value['field'] ) == $_POST['field'] ){
				die( (string)$value['id'] );
			}
		}
	}else
			...
			jQuery(document).ready(function() {
				jQuery(document).on( 'click', '#publish', function(){
					...
					if ( jQuery.trim( post_title ) == '' ){
						alert ( '<?php _e( 'You need to specify the title of the form before creating it', 'profilebuilder' ); ?>' );
...
function wppb_rf_epf_set_field_ids_on_field_select() {
    ...
    if( ( isset( $_GET['post_type'] ) && $_GET['post_type'] == 'wppb-epf-cpt' ) || ( isset( $_GET['post'] ) && get_post_type( $_GET['post'] ) == 'wppb-epf-cpt'  ) || ( isset( $_POST['meta'] ) && $_POST['meta'] == 'wppb_epf_fields' ) ) {
    ...
    if( ( isset( $_GET['post_type'] ) && $_GET['post_type'] == 'wppb-rf-cpt' ) || ( isset( $_GET['post'] ) && get_post_type( $_GET['post'] ) == 'wppb-rf-cpt'  ) || ( isset( $_POST['meta'] ) && $_POST['meta'] == 'wppb_rf_fields' ) ) {
        foreach( $all_fields as $key => $field ) {
