...
add_filter( 'wck_update_container_class_wppb_ul_page_settings', 'wppb_userlisting_update_form_change_class_based_on_visible_field', 10, 4 );
function wppb_userlisting_update_form_change_class_based_on_visible_field($wck_update_container_css_class, $meta, $results, $element_id) {
    if( !empty( $results[$element_id]["visible-only-to-logged-in-users"] ) )
        $votliu_val = $results[$element_id]["visible-only-to-logged-in-users"];
    else
        $votliu_val = '';
    $votliu = Wordpress_Creation_Kit_PB::wck_generate_slug( $votliu_val );
    return "class='update_container_$meta update_container_$votliu visible_to_logged_$votliu'";
}
add_filter( 'wck_element_class_wppb_ul_page_settings', 'wppb_userlisting_form_listing_change_class_based_on_visible_field', 10, 4 );
function wppb_userlisting_form_listing_change_class_based_on_visible_field($wck_element_class, $meta, $results, $element_id){
    if( !empty( $results[$element_id]["visible-only-to-logged-in-users"] ) )
        $votliu_val = $results[$element_id]["visible-only-to-logged-in-users"];
    else
        $votliu_val = '';
    $votliu = Wordpress_Creation_Kit_PB::wck_generate_slug( $votliu_val );
    $wck_element_class = "class='visible_to_logged_$votliu'";
	...
	if ( $wppb_manage_fields != 'not_found' )
		foreach( $wppb_manage_fields as $key => $value ){
            ...
            elseif ( $value['field'] == 'Default - Biographical Info' )
                $merge_tags[] = array( 'name' => $type.'_biographical_info', 'type' => $default_field_type, 'label' => __( 'Biographical Info', 'profilebuilder' ) );

            elseif ( $value['field'] == 'Upload' ){
				$merge_tags[] = array( 'name' => $type.'_'.$value['meta-name'], 'type' => $user_meta, 'label' => $value['field-title'] );
				$merge_tags[] = array( 'name' => $type.'_'.$value['meta-name'].'_URL', 'type' => $user_meta, 'label' => $value['field-title'] .' '. __( 'Url', 'profilebuilder' ));
...
<table class="wppb-table">
	<thead>
		<tr>
		  <th scope="col" colspan="2">{{{sort_user_name}}}</th>
		  <th scope="col">{{{sort_first_name}}}</th>
		  <th scope="col">Role</th>
		  <th scope="col">{{{sort_number_of_posts}}}</th>
		  <th scope="col">{{{sort_registration_date}}}</th>
...
function wppb_userlisting_show_default_user_fields( $value, $name, $children, $extra_info ){
	...
	if( $name == 'meta_user_name' ){
        ...
        if( !empty( $user_info->roles ) ){
            $user_roles_labels = get_option( 'wp_user_roles' );
            $current_user_role_labels = array();
            foreach( $user_info->roles as $role ){
                if( !empty( $user_roles_labels[$role]['name'] ) )
                    $current_user_role_labels[] = $user_roles_labels[$role]['name'];
            }
            return apply_filters('wppb_userlisting_extra_meta_role', implode( ', ', $current_user_role_labels ), $user_info );
        }
...
function wppb_userlisting_sort_tags( $value, $name, $children, $extra_info ){
	...
	if ( $name == 'sort_user_name' )
        return '<a href="'.wppb_get_new_url( 'login', $extra_info ).'" class="sortLink" id="sortLink1">'.apply_filters( 'sort_user_name_filter', __( 'Username', 'profilebuilder' ) ).'</a>';
	...
	elseif ( $name == 'sort_email' )
        return '<a href="'.wppb_get_new_url( 'email', $extra_info ).'" class="sortLink" id="sortLink2">'.apply_filters( 'sort_email_filter', __( 'Email', 'profilebuilder' ) ).'</a>';
		
	elseif ( $name == 'sort_registration_date' )
        return '<a href="'.wppb_get_new_url( 'registered', $extra_info ).'" class="sortLink" id="sortLink3">'.apply_filters( 'sort_registration_date_filter', __( 'Sign-up Date', 'profilebuilder' ) ).'</a>';
		
	elseif ( $name == 'sort_first_name' )
        return '<a href="'.wppb_get_new_url( 'firstname', $extra_info ).'" class="sortLink" id="sortLink4">'.apply_filters( 'sort_first_name_filter', __( 'Firstname', 'profilebuilder' ) ).'</a>';
		
	elseif ( $name == 'sort_last_name' )
        return '<a href="'.wppb_get_new_url( 'lastname', $extra_info ).'" class="sortLink" id="sortLink5">'.apply_filters( 'sort_last_name_filter', __( 'Lastname', 'profilebuilder' ) ).'</a>';
		
	elseif ( $name == 'sort_display_name' )		
        return '<a href="'.wppb_get_new_url( 'nicename', $extra_info ).'" class="sortLink" id="sortLink6">'.apply_filters( 'sort_display_name_filter', __( 'Display Name', 'profilebuilder' ) ).'</a>';
		
	elseif ( $name == 'sort_website' )
		return '<a href="'.wppb_get_new_url( 'url', $extra_info ).'" class="sortLink" id="sortLink7">'.apply_filters('sort_website_filter', __( 'Website', 'profilebuilder' ) ).'</a>';
	
	elseif ( $name == 'sort_biographical_info' )
        return '<a href="'.wppb_get_new_url( 'bio', $extra_info ).'" class="sortLink" id="sortLink8">'.apply_filters( 'sort_biographical_info_filter', __( 'Biographical Info', 'profilebuilder' ) ).'</a>';
		
	elseif ( $name == 'sort_number_of_posts' )
        return '<a href="'.wppb_get_new_url( 'post_count', $extra_info ).'" class="sortLink" id="sortLink9">'.apply_filters( 'sort_number_of_posts_filter', __( 'Posts', 'profilebuilder' ) ).'</a>';
		
	elseif ( $name == 'sort_aim' )
        return '<a href="'.wppb_get_new_url( 'aim', $extra_info ).'" class="sortLink" id="sortLink10">'.apply_filters( 'sort_aim_filter', __( 'Aim', 'profilebuilder' ) ).'</a>';
	
	elseif ( $name == 'sort_yim' )
        return '<a href="'.wppb_get_new_url( 'yim', $extra_info ).'" class="sortLink" id="sortLink11">'.apply_filters( 'sort_yim_filter', __( 'Yim', 'profilebuilder' ) ).'</a>';
	
	elseif ( $name == 'sort_jabber' )
        return '<a href="'.wppb_get_new_url( 'jabber', $extra_info ).'" class="sortLink" id="sortLink12">'.apply_filters( 'sort_jabber_filter', __( 'Jabber', 'profilebuilder' ) ).'</a>';

    elseif ( $name == 'sort_nickname' )
        return '<a href="'.wppb_get_new_url( 'nickname', $extra_info ).'" class="sortLink" id="sortLink13">'.apply_filters( 'sort_nickname_filter', __( 'Nickname', 'profilebuilder' ) ).'</a>';

    elseif ( $name == 'sort_role' )
        return '<a href="'.wppb_get_new_url( 'role', $extra_info ).'" class="sortLink" id="sortLink14">'.apply_filters( 'sort_role_filter', __( 'Role', 'profilebuilder' ) ).'</a>';
