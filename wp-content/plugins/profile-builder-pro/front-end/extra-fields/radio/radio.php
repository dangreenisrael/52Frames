...
function wppb_radio_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){
	if ( $field['field'] == 'Radio' ){
			...
			$output = '
				<label for="'.$field['meta-name'].'">'.$item_title.$error_mark.'</label>';
					...
					foreach( $radio_values as $key => $value){
						$output .= '<li><input value="'.trim( $value ).'" class="custom_field_radio" id="'.trim( $value ).'_'.$field['id'].'" name="'.$field['meta-name'].'" type="radio"';
						
						if ( $input_value === trim( $value ) )
							$output .= ' checked';

						$output .= ' /><label for="'.trim( $value ).'_'.$field['id'].'" class="wppb-rc-value">'.( ( !isset( $radio_labels[$key] ) || !$radio_labels[$key] ) ? trim( $radio_values[$key] ) : trim( $radio_labels[$key] ) ).'</label></li>'; 
					}
					$output .= '</ul>';
				...
				<table class="form-table">
					<tr>
						...
						<td>';
							...
							foreach( $radio_values as $key => $value ){								
								$output .= '<li><input value="'.trim( $value ).'"  id="'.trim( $value ).'_'.$field['id'].'" class="custom_field_radio" name="'.$field['meta-name'].'" type="radio"';
		...
		return apply_filters( 'wppb_'.$form_location.'_radio_custom_field_'.$field['id'], $output, $form_location, $field, $user_id, $field_check_errors, $request_data );
	}
}
...
function wppb_check_radio_value( $message, $field, $request_data, $form_location ){
	if( $field['field'] == 'Radio' ){
		if ( !isset( $request_data[$field['meta-name']] ) && ( $field['required'] == 'Yes' ) ){
