...
function wppb_multiple_select_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){
	if ( $field['field'] == 'Select (Multiple)' ){
        ...
        $input_value = ( isset( $request_data[$field['meta-name']] ) ? array_map( 'trim', $request_data[$field['meta-name']] ) : $input_value );
		...
		if ( $form_location != 'back_end' ){
			...
			$output = '
				...
				<select name="'.$field['meta-name'].'[]" id="'.$field['meta-name'].'" size="'.( count( $select_values ) > 10 ? count( $select_values ) / 2 : count( $select_values ) ).'" class="custom_field_multiple_select" multiple="multiple">';
			...
			$output = '
				<table class="form-table">
					<tr>
						...
						<td>
							<select name="'.$field['meta-name'].'[]" class="custom_field_multiple_select" id="'.$field['meta-name'].'" multiple="multiple">';
		...
		return apply_filters( 'wppb_'.$form_location.'_multiple_select_custom_field_'.$field['id'], $output, $form_location, $field, $user_id, $field_check_errors, $request_data );
	}
}
...
function wppb_process_multipl_select_value( $field, $request_data ){
	...
	if( !empty( $request_data[$field['meta-name']] ) ){
		foreach ( $request_data[$field['meta-name']] as $key => $value )
