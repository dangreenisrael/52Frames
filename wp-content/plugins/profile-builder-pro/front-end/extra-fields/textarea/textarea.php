...
function wppb_textarea_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){
	if ( $field['field'] == 'Textarea' ){
        ...
        if( $form_location != 'register' )
            ...
            $input_value = ( !empty( $field['default-content'] ) ? trim( $field['default-content'] ) : '' );
		...
		if ( $form_location != 'back_end' ){
			...
			$output = '
				...
				<textarea rows="'.$field['row-count'].'" name="'.$field['meta-name'].'" maxlength="'. apply_filters( 'wppb_maximum_character_length', '', $field ) .'" class="custom_field_textarea" id="'.$field['meta-name'].'" wrap="virtual">'. esc_textarea( wp_unslash( $input_value ) ) .'</textarea>';
			...
			$output = '
				<table class="form-table">
					<tr>
						...
						<td>
							<textarea rows="'.$field['row-count'].'" name="'.$field['meta-name'].'" maxlength="'. apply_filters( 'wppb_maximum_character_length', '', $field ) .'" class="custom_field_textarea" id="'.$field['meta-name'].'" wrap="virtual">'. esc_textarea( $input_value ) .'</textarea>
		...
		return apply_filters( 'wppb_'.$form_location.'_textarea_custom_field_'.$field['id'], $output, $form_location, $field, $user_id, $field_check_errors, $request_data );
	}
}
