...
function wppb_agree_to_terms_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){
	if ( $field['field'] == 'Checkbox (Terms and Conditions)' ){
		if ( $form_location == 'register' ){
			...
			$input_value = ( isset( $request_data[$field['meta-name']] ) ? trim( $request_data[$field['meta-name']] ) : '' );
			...
			$output = '
				...
				<input value="agree" name="'.$field['meta-name'].'" id="'.$field['meta-name'].'" type="checkbox" class="custom_field_toa"';
			...
			return apply_filters( 'wppb_'.$form_location.'_toa_custom_field_'.$field['id'], $output, $form_location, $field, $user_id, $field_check_errors, $request_data );
		}
	}
}
...
function wppb_check_toa_value( $message, $field, $request_data, $form_location ){
	if( $field['field'] == 'Checkbox (Terms and Conditions)' ){
		if ( $form_location == 'register' ){
			if ( ( ( isset( $request_data[$field['meta-name']] ) && ( trim( $request_data[$field['meta-name']] ) == '' ) ) && ( $field['required'] == 'Yes' ) ) || ( !isset( $request_data[$field['meta-name']] ) && ( $field['required'] == 'Yes' ) ) ){
