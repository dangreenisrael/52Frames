<?php
/*
Plugin Name: 52Frames Recent Posts Slider
	...
	foreach ( $recent_posts as $key=>$val ) {
		...
		if ( $slider_content == 2 ) {
			if ( !empty($val->post_excerpt) ) 
				$post_details[$key]['post_excerpt'] = create_excerpt($val->post_excerpt, $excerpt_length, $post_details[$key]['post_permalink'], $excerpt_words, $val->ID);
			else
				$post_details[$key]['post_excerpt'] = create_excerpt($val->post_content, $excerpt_length, $post_details[$key]['post_permalink'], $excerpt_words, $val->ID);
		}elseif ( $slider_content == 1 ) {
			$post_details[$key]['post_first_img'] = get_post_meta($val->ID, '_rps_img_src');
		}elseif ( $slider_content == 3 ) {
			$post_details[$key]['post_first_img'] = get_post_meta($val->ID, '_rps_img_src');
			if ( !empty($val->post_excerpt) ) 
				$post_details[$key]['post_excerpt'] = create_excerpt($val->post_excerpt, ($excerpt_length/2)-10, $post_details[$key]['post_permalink'], $excerpt_words, $val->ID);
			else
				$post_details[$key]['post_excerpt'] = create_excerpt($val->post_content, ($excerpt_length/2)-10, $post_details[$key]['post_permalink'], $excerpt_words, $val->ID);
		}
		if ( $show_post_date ){
			...
			$output .= '<div class="slide">';
					for ( $j = 1; $j <= $post_per_slide; $j++ ) {
						...
						if ( $slider_content == 2 ){
							...
							$output .= '<div class="slider-content-both">';
							
							if( !empty($post_details[$p]['post_first_img']['0']) || !empty($post_details[$p]['post_excerpt'])){
								$rps_img_src_path = $post_details[$p]['post_first_img']['0'];
								if(!empty($rps_img_src_path)){
									$output .= '<p class="slider-img-hp"><a href="'.$post_details[$p]['post_permalink'].'"><img src="'.$rps_img_src_path.'" alt="'.__($post_details[$p]['post_title'], 'rps').'" align="left" /></a></p>';
								}
								$output .= '<div class="slider-content-both-wrap">';
								$output .= '<p class="slider-title-hp"><a href="'.$post_details[$p]['post_permalink'].'"><span>'.__($post_details[$p]['post_title'], 'rps').'</span></a></p><p class="slider-content-hp">'.__($post_details[$p]['post_excerpt'], 'rps').'</p>';
							}
							if($show_post_date){
								$output .= '<div class="post-date">'.__($post_date_text, 'rps').' '.__($post_details[$p]['post_date'], 'rps').'</div>';
							}
...
*/
function create_excerpt( $post_content, $excerpt_length, $post_permalink, $excerpt_words=NULL, $key){
	...
	if( !empty($excerpt_words) ){	
		if ( !empty($post_excerpt) ) {
			...
			array_pop($words);
			array_push($words, ' <br /><a class="slider-button-hp" href="'.get_field('free_link', $key ).'">'.$more_link.'</a>');
		...
		if ( !empty($post_excerpt_rps) ) {
			...
			$post_excerpt_rps .= ' <a class="slider-button-hp" href="'.get_field( 'free_link', $key ).'">'.$more_link.'</a>';
			return $post_excerpt_rps;
		} else {
			return;
		}
