...
<div id="module-manager-wrap" class="wrap">
    ...
    if (( $toolset_any_embedded ) || ($toolset_all_disabled)) {
		...
		<div class="error">
			<p>
				<?php _e( 'Exporting and importing modules disable. You will need to activate Toolset plugins full versions ( Types, Views and CRED ) in order to enable this feature.', 'module-manager' ); ?>				
			</p>
		</div>		
		<?php } elseif ((!($toolset_all_installed)) && (!($toolset_any_embedded))) {?>
		<div class="error">
			<p>
				<?php _e( 'Exporting and importing modules disable. You will need to activate one of the Toolset full plugin versions ( Types, Views and CRED ) in order to enable this feature.', 'module-manager' ); ?>				
			</p>
		</div>
		<?php } else {?>
		<div class="error">
			<p>
				<?php _e( 'Exporting modules are disabled. You are using embbedded versions of the Toolset plugins.', 'module-manager' ); ?>				
			</p>
		</div>		
		<?php }?>		
		<?php
