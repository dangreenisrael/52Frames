...
if (!function_exists('wpbootstrap_get_content_width')) {
    ...
    function wpbootstrap_get_content_width() {
        ...
        if (wpbootstrap_get_setting('general_settings', 'display_sidebar') && ((!is_page_template('page-fullwidth.php')) && (!is_page_template('page-home.php')))) {
...
}// Custom Fields$first_challenge = types_render_field("this-is-not-my-first-chalange", array("show_name"=>"true","output"=>"html","id"=>"first-challenge"));
...
if (!function_exists('wpbootstrap_nav_menu_defaults')) {
    ...
    function wpbootstrap_nav_menu_defaults($args = '') {
		...
		if( 'header-menu' == $args['theme_location'] ) {
            ...
            //Not header menu, return unfiltered args.
...
if (!function_exists('wpbootstrap_get_nav_menu_classes')) {
    ...
    function wpbootstrap_get_nav_menu_classes() {
        ...
        if ( of_get_option( 'navbar_style' ) === 'menu_static' ) {
            ...
            $wpbootstrap_navbar_classes = $wpbootstrap_navbar_classes . ' span8';
 ...
 * Register a widget in the alboms' page.

 *

 */

function alboms_widgets_init() {
...
add_action( 'widgets_init', 'alboms_widgets_init' );
...
if (!function_exists('wpbootstrap_register_scripts')) {
    ...
    function wpbootstrap_register_scripts() {
        ...
        if (!is_admin()) {
            ...
            wp_enqueue_style( 'toolset-bootstrap-theme-style', get_stylesheet_uri(),array(),'1.5' );
...
/* Make sure users are aware they are not duplicating their style.css after updating to Toolset Bootstrap 1.5 */

//START

if (!function_exists('wpbootstrap_detect_potential_duplicate_style_css')) {



	function wpbootstrap_detect_potential_duplicate_style_css() {



		//Check if using child theme and in the admin

		if ((is_child_theme()) && (is_admin())) {



			//Formulate path to header.php

			$header_php_path= get_stylesheet_directory() . '/header.php';

			$headerlayouts_php_path= get_stylesheet_directory() . '/header-layouts.php';



			if ( file_exists( $header_php_path ) && is_file( $header_php_path) ) {



				//User overrides header.php in child theme

				//Retrieved contents of this file

				$headerphp_data = implode( '', file( $header_php_path ) );



				if (strpos($headerphp_data,'stylesheet_url') !== false) {

		

					//OK instances of stylesheet_url is found header.php

					add_action('admin_notices','wpbootstrap_show_message_remove_duplicate_stylecss');
			...
			if ( file_exists( $headerlayouts_php_path ) && is_file( $headerlayouts_php_path) ) {
				...
				if (defined('WPDDL_VERSION')) {
					...
					//User overrides header-layouts.php in child theme

					//Retrieved contents of this file

					$headerlayoutsphp_data = implode( '', file( $headerlayouts_php_path ) );

						

					if (strpos($headerlayoutsphp_data,'stylesheet_url') !== false) {
	...
	add_action( 'after_setup_theme', 'wpbootstrap_detect_potential_duplicate_style_css',25);	

}



if (!function_exists('wpbootstrap_show_message_remove_duplicate_stylecss')) {    

    

	function wpbootstrap_show_message_remove_duplicate_stylecss() {

		?>

	    <div class="update-nag">

	        <p><?php _e( 'Toolset Bootstrap 1.5+ now loads style.css in functions.php. Please remove the link rel stylesheet in your Toolset Bootstrap child theme header.php or header-layouts.php if you have Layouts plugin activated.', 'wpbootstrap' );?></p>
