    ...
    public function havePermissions($code, $action) {
        ...
        if($mod) {
            ...
            if(!empty($permissions)) {  // Special permissions
                if(isset($permissions[CSP_METHODS]) 
                    && !empty($permissions[CSP_METHODS])
                    
                ) {
                    foreach($permissions[CSP_METHODS] as $method => $permissions) {   // Make case-insensitive
                        $permissions[CSP_METHODS][strtolower($method)] = $permissions;
                    }
                    if(array_key_exists($action, $permissions[CSP_METHODS])) {        // Permission for this method exists
                        ...
                        if((is_array($permissions[ CSP_METHODS ][ $action ]) && !in_array($currentUserPosition, $permissions[ CSP_METHODS ][ $action ]))
                            || (!is_array($permissions[ CSP_METHODS ][ $action ]) && $permissions[CSP_METHODS][$action] != $currentUserPosition)
                        ) {
                            $res = false;
                        }
                    }
                }
                if(isset($permissions[CSP_USERLEVELS])
                    && !empty($permissions[CSP_USERLEVELS])
                ) {
                    $currentUserPosition = frameCsp::_()->getModule('user')->getCurrentUserPosition();
                    ...
                    foreach($permissions[CSP_USERLEVELS] as $userlevel => $methods) {
                        if(is_array($methods)) {
